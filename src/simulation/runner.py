...
import sys
import os
import cv2  # Fix libtiff/libjpeg symbol conflict
import torch
import warp as wp
import numpy as np
import json
...
if __name__ == "__main__":
    ...
    if args.tags_path is not None and os.path.exists(args.tags_path):
        init_tags = torch.load(args.tags_path).to("cuda")
    else:
        init_tags = torch.zeros(init_pos.shape[0], dtype=torch.int32, device="cuda")
    ...
    unselected_pos, unselected_cov, unselected_opacity, unselected_shs, unselected_tags = (
        None,
        None,
        None,
        None,
    ...
    if preprocessing_params["sim_area"] is not None:
        ...
        unselected_tags = init_tags[~mask]
    ...
    if filling_params is not None:
        ...
        print("Assigning material tags to filled particles via Nearest Neighbor...")
        from scipy.spatial import KDTree
        tree = KDTree(transformed_pos.detach().cpu().numpy())
        filled_pos_np = mpm_init_pos[gs_num:].detach().cpu().numpy()
        if filled_pos_np.shape[0] > 0:
            _, indices = tree.query(filled_pos_np)
            filled_tags = init_tags[torch.from_numpy(indices).to(device)]
            mpm_init_tags = torch.cat([init_tags, filled_tags], dim=0)
        else:
            mpm_init_tags = init_tags
    else:
    ...
    # Map tags to per-particle material properties
    if "materials" in material_params and material_params["materials"] is not None:
        materials_dict = material_params["materials"]
        E_tensor = torch.full_like(mpm_init_tags, material_params.get("E", 1e5), dtype=torch.float32)
        nu_tensor = torch.full_like(mpm_init_tags, material_params.get("nu", 0.4), dtype=torch.float32)
        density_tensor = torch.full_like(mpm_init_tags, material_params.get("density", 200.0), dtype=torch.float32)
        for tag_id, props in materials_dict.items():
            mask = (mpm_init_tags == tag_id)
            E_tensor[mask] = props.get("E", 1e5)
            nu_tensor[mask] = props.get("nu", 0.4)
            density_tensor[mask] = props.get("density", 200.0)
            
        material_params["E_array"] = E_tensor
        material_params["nu_array"] = nu_tensor
        material_params["density_array"] = density_tensor
